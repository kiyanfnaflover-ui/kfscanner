...
func probeWebSocket(ctx context.Context, ip net.IP, port int, sni, host, path string, timeout time.Duration) bool {
	...
	// WebSocket over plain HTTP (port 80) has no TLS layer; the idle-hold check
	// below is TLS-specific and only applies to HTTPS ports.
	if port != 80 {
	...
	wsReq := fmt.Sprintf(
		"GET %s HTTP/1.1\r\n"+
			...
			"Upgrade: websocket\r\n"+
			"Connection: Upgrade\r\n"+
			"Sec-WebSocket-Key: a2ZzY2FubmVy\r\n"+
