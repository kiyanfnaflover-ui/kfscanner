...
func ValidateConfig(ctx context.Context, cfg *VLESSConfig, timeout time.Duration) *ValidationResult {
	...
	if !res.Success {
		// Retry once — DPI is flaky. Bail immediately if cancelled so a stopped
		// scan does not linger an extra half-second per in-flight validation.
		select {
		case <-ctx.Done():
			res.Retries = 0
			return res
...
func validateOnce(ctx context.Context, cfg *VLESSConfig, timeout time.Duration) *ValidationResult {
	...
	if !waitForPort(ctx, socksPort, 5*time.Second) {
...
// waitForPort waits until a TCP port is accepting connections. It returns as
// soon as the context is cancelled so a stopped scan never blocks on a port.
