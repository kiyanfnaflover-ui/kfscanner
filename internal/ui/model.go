...
import (
	...
	"github.com/kfscanner/kfscanner/internal/banner"
	"github.com/kfscanner/kfscanner/internal/config"
...
func getConfigFilePath() string {
	...
	if err != nil {
		return "kfscanner-config.json"
	}
	appDir := filepath.Join(dir, "kfscanner")
...
func (m AppModel) viewAbout() string {
	...
	sb.WriteString(banner.Render(m.bannerFrame / 2))
	sb.WriteRune('\n')
	sb.WriteString(styleTitle.Render("  KF Scanner\n"))
	sb.WriteString(styleDim.Render(fmt.Sprintf("  version %s", m.version)))
	sb.WriteString("\n\n")
	sb.WriteString(styleNormal.Render("  A Cloudflare endpoint scanner built for high-latency, restricted networks."))
	sb.WriteRune('\n')
	...
	sb.WriteString(styleDim.Render("  created by K_F_ · github.com/kfscanner/kfscanner"))
	sb.WriteString("\n\n")
...
func (m AppModel) exportAllConfigs() string {
	...
	subPath := filepath.Join(dir, "kfscanner-sub.txt")
	_ = os.WriteFile(subPath, []byte(bundle.Subscription), 0644)

	sbPath := filepath.Join(dir, "kfscanner-singbox.json")
	_ = os.WriteFile(sbPath, []byte(bundle.SingBox), 0644)

	clashPath := filepath.Join(dir, "kfscanner-clash.yaml")
