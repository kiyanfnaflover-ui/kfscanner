# Contributing to KF Scanner

Thank you for taking the time to improve KF Scanner. This project exists so that people on slow or restricted networks can find Cloudflare IPs that **actually work with their own config** — without reading docs, memorizing flags, or babysitting a terminal.
...
KF Scanner is **not** a general-purpose network lab. Resist scope creep: CLI flags, export formats, and power-user toggles only belong here when they serve the main workflow.
...
```bash
git clone https://github.com/K_F_/KFScanner.git
cd kfscanner
go mod download
make build          # → ./kfscanner
...
| Version | output of `kfscanner --version` |
...
Open a [GitHub Discussion](https://github.com/K_F_/KFScanner/discussions) or issue if you are unsure whether an idea fits. A quick “is this in scope?” question saves everyone time.

Thank you for helping keep KF Scanner fast, simple, and reliable for the people who actually need it.
